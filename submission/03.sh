#!/bin/bash
# Create a 1-of-4 P2SH multisig address from the public keys in the four inputs of this raw tx:
transaction=""

PUB1=$(bitcoin-cli -regtest decoderawtransaction "$transaction" | jq -r '.vin[0].txinwitness[1]')
PUB2=$(bitcoin-cli -regtest decoderawtransaction "$transaction" | jq -r '.vin[1].txinwitness[1]')
PUB3=$(bitcoin-cli -regtest decoderawtransaction "$transaction" | jq -r '.vin[2].txinwitness[1]')
PUB4=$(bitcoin-cli -regtest decoderawtransaction "$transaction" | jq -r '.vin[3].txinwitness[1]')

bitcoin-cli -regtest createmultisig 1 "[\"$PUB1\",\"$PUB2\",\"$PUB3\",\"$PUB4\"]" "legacy" | jq -r '.address'
